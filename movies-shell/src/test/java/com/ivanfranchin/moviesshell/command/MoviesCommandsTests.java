...
class MoviesCommandsTests {
    ...
    public void getMovies_whenApiReturnsMovies_thenOutputContainsStatusAndBody() throws Exception {
        MovieResponse movie1 = new MovieResponse("tt0111161", "The Shawshank Redemption", "Frank Darabont", 1994);
        MovieResponse movie2 = new MovieResponse("tt0068646", "The Godfather", "Francis Ford Coppola", 1972);
        when(moviesApiClient.getMovies())
                .thenReturn(ResponseEntity.ok(List.of(movie1, movie2)));
    ...
    @Test
    public void getMovies_whenApiReturnsEmptyList_thenOutputContainsStatusAndEmptyList() throws Exception {
        when(moviesApiClient.getMovies())
    ...
    public void getMovie_whenMovieExists_thenOutputContainsStatusAndMovie() throws Exception {
        MovieResponse movie = new MovieResponse("tt0111161", "The Shawshank Redemption", "Frank Darabont", 1994);
        when(moviesApiClient.getMovie("tt0111161"))
    ...
    public void getMovie_whenMovieDoesNotExist_thenOutputContainsErrorMessage() throws Exception {
        when(moviesApiClient.getMovie("tt9999999"))
    ...
    @Test
    public void addMovie_whenRequestIsValid_thenOutputContainsStatusAndCreatedMovie() throws Exception {
        MovieResponse created = new MovieResponse("tt0111161", "The Shawshank Redemption", "Frank Darabont", 1994);
        when(moviesApiClient.addMovie(any(AddMovieRequest.class)))
                .thenReturn(ResponseEntity.status(HttpStatus.CREATED).body(created));

        ShellAssertions.assertThat(shellTestClient.sendCommand(
    ...
    public void deleteMovie_whenMovieDoesNotExist_thenOutputContainsErrorMessage() throws Exception {
        when(moviesApiClient.deleteMovie("tt9999999"))
                .thenThrow(new RuntimeException("404 Not Found"));
